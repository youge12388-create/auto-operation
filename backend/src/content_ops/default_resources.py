...
DEFAULT_SKILLS = [{'name': 'khazix-writer',
  ...
  'prompt': '---\n'
            ...
            '2. **过度结构化**：不用bullet '
            'point罗列观点，不大量加粗。卡兹克的文章绝大多数时候没有小标题，从头到尾一口气顺下来，靠节奏和转场自然推进。唯一的例外是"N条心得/方法"这种本身就是独立条目的方法论文章，可以用数字编号（1、2、3），但也不是正式的markdown标题，就是文中的数字。如果不是这种分条目的结构，就不要加小标题，用口语化的转场句（"说到这个""回到xxx这块"）来衔接板块\n'
            '3. **标点禁令**：\n'
            '   - 不使用冒号"："，用逗号代替\n'
            '   - 不使用破折号"——"\n'
