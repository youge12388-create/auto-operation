...
- Production-line runs execute the fixed content workflow through article rendering and local draft creation. When `human_review_required` is enabled, the job pauses at `waiting_review`; formal WeChat publication remains a separate explicit action.
...
- Skill and model choices belong to production-line configuration before generation. Theme, cover, and channel choices belong to approved-article delivery after review.
...
`content_ops.wechat.WeChatClient` 通过官方 HTTPS API 处理 access_token、永久封面素材、草稿创建、草稿获取和草稿更新。API 路由位于 `/api/v1/channels/wechat/*` 与文章修订版本下的 `/wechat-draft` 路径。公众号全局凭证从 `backend/.env` 读取；绑定到频道的凭证使用应用级密钥加密存储，接口和日志不返回完整密钥；发布动作仍保持人工确认。
