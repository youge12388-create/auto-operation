...
function FigmaTopbar({ page, user, notificationCount, onSearch, onLogout, onChangePassword }: { page: Page; user: User; notificationCount: number; onSearch: (value: string) => void; onLogout: () => void; onChangePassword: () => void }) {
  ...
  return <header className="figma-topbar"><div className="figma-topbar-title"><strong>内容运营系统</strong><span>/</span><span>{label}</span></div><label className="figma-search"><Icon name="search" size={17} /><input placeholder="搜索内容、选题或素材..." onKeyDown={(event) => { if (event.key === "Enter") onSearch(event.currentTarget.value); }} /></label><div className="figma-topbar-actions"><div className="figma-topbar-menu"><button type="button" aria-label="通知" aria-expanded={open === "notifications"} onClick={() => setOpen(open === "notifications" ? null : "notifications")}><Icon name="bell" size={19} />{notificationCount > 0 && <i>{notificationCount}</i>}</button>{open === "notifications" && <div className="figma-topbar-popover"><strong>待处理提醒</strong><p>有 {notificationCount || 0} 个任务需要关注。</p><button type="button" onClick={() => setOpen(null)}>知道了</button></div>}</div><div className="figma-topbar-menu"><button type="button" aria-label="帮助" aria-expanded={open === "help"} onClick={() => setOpen(open === "help" ? null : "help")}><Icon name="help" size={19} /></button>{open === "help" && <div className="figma-topbar-popover"><strong>帮助中心</strong><p>先从素材池筛选依据，再确认选题、配置策略，最后审核并创建公众号草稿。</p><button type="button" onClick={() => setOpen(null)}>关闭</button></div>}</div><div className="figma-topbar-menu"><button className="figma-avatar" type="button" aria-label="账户菜单" aria-expanded={open === "profile"} onClick={() => setOpen(open === "profile" ? null : "profile")}>{user.email.slice(0, 1).toUpperCase()}</button>{open === "profile" && <div className="figma-topbar-popover figma-profile-popover"><strong>{user.email}</strong><span>{user.role === "admin" ? "管理员" : "运营成员"}</span><button type="button" onClick={() => { setOpen(null); onChangePassword(); }}><Icon name="lock" size={14} />修改密码</button><button type="button" onClick={onLogout}>退出登录</button></div>}</div></div></header>;
}

type PasswordChangePayload = { current_password: string; new_password: string; confirm_password: string };

function ChangePasswordModal({ open, submitting, onClose, onSubmit }: { open: boolean; submitting: boolean; onClose: () => void; onSubmit: (payload: PasswordChangePayload) => Promise<unknown> }) {
  const [form, setForm] = useState<PasswordChangePayload>({ current_password: "", new_password: "", confirm_password: "" });
  const [error, setError] = useState("");

  useEffect(() => { if (open) { setForm({ current_password: "", new_password: "", confirm_password: "" }); setError(""); } }, [open]);
  if (!open) return null;
  const submit = async () => {
    if (!form.current_password) { setError("请输入当前密码。"); return; }
    if (form.new_password.length < 12) { setError("新密码至少需要 12 位。"); return; }
    if (form.new_password !== form.confirm_password) { setError("两次输入的新密码不一致。"); return; }
    setError("");
    try { await onSubmit(form); } catch (reason) { setError(reason instanceof Error ? reason.message : "修改密码失败，请稍后重试。"); }
  };

  return <div className="figma-modal-backdrop" onClick={(event) => { if (event.target === event.currentTarget && !submitting) onClose(); }}><div className="figma-modal password-change-modal" role="dialog" aria-modal="true" aria-labelledby="password-change-title"><button className="modal-close" type="button" aria-label="关闭" disabled={submitting} onClick={onClose}><Icon name="close" size={18} /></button><h2 id="password-change-title">修改密码</h2><p className="password-change-note"><Icon name="shield" size={16} />保存后会退出当前账号，其他设备上的旧登录也将失效。</p><form onSubmit={(event) => { event.preventDefault(); void submit(); }}><label>当前密码<input type="password" autoComplete="current-password" value={form.current_password} onChange={(event) => setForm((current) => ({ ...current, current_password: event.target.value }))} required /></label><label>新密码<input type="password" autoComplete="new-password" minLength={12} maxLength={200} value={form.new_password} onChange={(event) => setForm((current) => ({ ...current, new_password: event.target.value }))} required /></label><label>确认新密码<input type="password" autoComplete="new-password" minLength={12} maxLength={200} value={form.confirm_password} onChange={(event) => setForm((current) => ({ ...current, confirm_password: event.target.value }))} required /></label>{error && <div className="form-error" role="alert">{error}</div>}<div className="modal-form-actions"><PillButton type="button" onClick={onClose} disabled={submitting}>取消</PillButton><PillButton type="submit" tone="pink" disabled={submitting}>{submitting ? "保存中…" : "保存并退出"}</PillButton></div></form></div></div>;
}
...
export function FigmaConsole({ currentUser }: { currentUser: User }) {
  ...
  const addUser = useMutation({ mutationFn: api.addUser, onSuccess: () => { void refresh("users"); message.success("用户已添加"); }, onError: (error: Error) => message.error(error.message) });
  const changePassword = useMutation({ mutationFn: api.changePassword, onSuccess: () => { setPasswordChangeOpen(false); message.success("密码已更新，请使用新密码重新登录。"); window.setTimeout(() => window.location.reload(), 500); } });
