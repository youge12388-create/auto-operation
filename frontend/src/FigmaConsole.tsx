...
function Topics({ topics, onConfirm, onNew, onEditor }: { topics: Topic[]; onConfirm: (topic: Topic) => void; onNew: () => void; onEditor: () => void }) {
  ...
  return <main className="figma-page topics-page"><div className="figma-page-heading"><div><h1>选题与创作</h1><p>浏览 AI 为您量身定制的热门选题，或直接开始创作。</p></div><PillButton tone="pink" onClick={onNew}><Icon name="edit" size={16} />手动创建选题</PillButton></div><div className="figma-tabs"><button className="is-active" type="button">候选选题 <b>{candidate.length || 12}</b></button><button type="button" onClick={onEditor}>文章创作 <b>→</b></button></di…18199 tokens truncated…runStrategy = useMutation({ mutationFn: ({ id, combinationId }: { id: string; combinationId?: string }) => api.runStrategy(id, combinationId), onSuccess: () => { message.success("自动化任务已启动：将自动采集、分类、精选、选题、写作、审核并按交付模式处理"); void refresh("jobs", "articles", "materials", "topics"); }, onError: (error: Error) => message.error(error.message) });
